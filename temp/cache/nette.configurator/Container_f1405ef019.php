...
class Container_f1405ef019 extends Nette\DI\Container
{
	...
	{
		return new App\Model\BookApiService('AIzaSyCK7ZVlj0v-UpXmwBNtTw4TPzljvNKUnnw', $this->getService('03'));
	}


	public function createService03(): App\Model\FileManager
	{
		return new App\Model\FileManager('/var/www/mp/www');
	}
	...
	{
		$service = new App\UI\Test\TestPresenter($this->getService('02'));
		$service->injectPrimary(
			$this->getService('http.request'),
			$this->getService('http.response'),
			$this->getService('application.presenterFactory'),
	...
	public function createServiceLatte__latteFactory(): Nette\Bridges\ApplicationLatte\LatteFactory
	{
		return new class ($this) implements Nette\Bridges\ApplicationLatte\LatteFactory {
			public function __construct(
				private Container_f1405ef019 $container,
			) {
			}
